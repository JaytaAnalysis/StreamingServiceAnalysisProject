SELECT 
  ...
  ROUND(AVG(LENGTH(title))/AVG(LENGTH(my_table)),1) AS avg_time,
  /*CASE
    WHEN STDDEV(LENGTH(my_table)) IS NULL THEN 0
    WHEN STDDEV(LENGTH(my_table)) IS NOT NULL THEN ROUND(STDDEV(LENGTH(title)/LENGTH(my_table)),1)
  END AS stddev_time*/
FROM 
  `asymmetric-moon-377217.streaming_libraries.full_library`, UNNEST(SPLIT(title,' ')) AS my_table
/* Ignore any titles where the table does not indicate if it is a movie or a tv show */
WHERE duration_type = 'min' AND title IS NOT NULL AND date_added IS NOT NULL
GROUP BY year, month
ORDER BY year, month
